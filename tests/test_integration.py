"""End-to-end integration tests: a real asyncio WebSocket server driven by scripted
WebSocket clients (no coinche.ui/terminal involved).
...
async def _start_server(target_score: int = 1000) -> tuple[Server, int]:
    async def _handler(websocket: ServerConnection) -> None:
        ...
        await server.handle_connection(websocket, target_score, trick_pause_seconds=0, round_pause_seconds=0)

    srv = await serve(_handler, HOST, 0, max_size=server.MAX_MESSAGE_SIZE)
...
async def _send(websocket: ClientConnection, msg_type: str, payload: dict) -> None:
    await websocket.send(protocol.encode(msg_type, payload))


async def _recv(websocket: ClientConnection, timeout: float = 5.0) -> tuple[str, dict]:
    try:
        message = await asyncio.wait_for(websocket.recv(), timeout=timeout)
    except ConnectionClosed:
        message = None
    assert message, "connection closed unexpectedly while awaiting a message"
    return protocol.decode(message)


async def _read_until(websocket: ClientConnection, msg_type: str) -> dict:
    """Read from `websocket` until a message of `msg_type` arrives, ignoring others."""
    while True:
        mtype, payload = await _recv(websocket)
...
async def _read_any_until(websocket: ClientConnection, msg_type: str) -> dict:
...
async def _assert_no_message(websocket: ClientConnection, timeout: float = 0.3) -> None:
    with pytest.raises(asyncio.TimeoutError):
        await asyncio.wait_for(websocket.recv(), timeout=timeout)


async def _join_all(port: int, table_key: str) -> dict[str, ClientConnection]:
    """Connect and join 4 scripted clients in seat order N, E, S, W."""
    conns: dict[str, ClientConnection] = {}
    for seat_key in SEAT_JOIN_ORDER:
        websocket = await _connect(port)
        await _send(websocket, protocol.JOIN, {"table_key": table_key, "player_name": NAMES_BY_SEAT[seat_key]})
        mtype, payload = await _recv(websocket)
        assert mtype == protocol.JOINED
        assert payload["seat"] == seat_key
        conns[seat_key] = websocket
    return conns


async def _bid_min_and_finalize_contract(conns: dict, observer: ClientConnection) -> dict:
    ...
    bid_req = await _read_until(conns["W"], protocol.BID_REQUEST)
    assert bid_req["legal_actions"], "first bidder must have at least one legal bid"
    action = bid_req["legal_actions"][0]
    await _send(conns["W"], protocol.BID, {"action": "bid", "trump": action["trump"], "points": action["points"]})

    update = await _read_until(observer, protocol.BID_UPDATE)
    assert update["seat"] == "W" and update["action"] == "bid"
    next_seat = update["next_to_act"]

    for _ in range(2):
        await _read_until(conns[next_seat], protocol.BID_REQUEST)
        await _send(conns[next_seat], protocol.BID, {"action": "pass"})
        update = await _read_until(observer, protocol.BID_UPDATE)
    ...
    await _read_until(conns[next_seat], protocol.BID_REQUEST)
    await _send(conns[next_seat], protocol.BID, {"action": "pass"})
    result = await _read_until(observer, protocol.BIDDING_RESULT)
...
def test_full_round_join_deal_bid_trick_score_flow():
    async def scenario() -> None:
        ...
        try:
            ...
            deal_payload = await _read_until(conns["W"], protocol.DEAL)
            assert len(deal_payload["hand"]) == 8
            assert deal_payload["dealer_seat"] == "N"
            assert deal_payload["first_bidder_seat"] == "W"

            await _bid_min_and_finalize_contract(conns, observer)

            current_actor = "W"
            for trick_num in range(1, 9):
                for _ in range(4):
                    req = await _read_until(conns[current_actor], protocol.PLAY_REQUEST)
                    assert req["legal_cards"], f"{current_actor} must have a legal card to play"
                    card = req["legal_cards"][0]
                    await _send(conns[current_actor], protocol.PLAY_CARD, {"card": card})
                    played = await _read_until(observer, protocol.CARD_PLAYED)
                    assert played["seat"] == current_actor
                    current_actor = ROTATION_NEXT[current_actor]

                trick_result = await _read_until(observer, protocol.TRICK_RESULT)
            ...
            round_score = await _read_until(observer, protocol.ROUND_SCORE)
...
def test_disconnect_and_reconnect_mid_round():
    async def scenario() -> None:
        ...
        try:
            ...
            await _read_until(conns["W"], protocol.DEAL)
            await _bid_min_and_finalize_contract(conns, observer)

            # Trick 1: W and S play normally.
            for seat in ("W", "S"):
                req = await _read_until(conns[seat], protocol.PLAY_REQUEST)
                card = req["legal_cards"][0]
                await _send(conns[seat], protocol.PLAY_CARD, {"card": card})
                played = await _read_until(observer, protocol.CARD_PLAYED)
            ...
            await _read_until(conns["E"], protocol.PLAY_REQUEST)
            e_websocket = conns["E"]
            await e_websocket.close()

            status = await _read_until(observer, protocol.CONNECTION_STATUS)
            ...
            await _assert_no_message(observer)

            # A new connection re-joins with the same table_key + player_name.
            new_ws = await _connect(port)
            await _send(new_ws, protocol.JOIN, {"table_key": "recon01", "player_name": NAMES_BY_SEAT["E"]})

            resync = await _read_until(new_ws, protocol.RESYNC)
            ...
            reconnected_status = await _read_until(observer, protocol.CONNECTION_STATUS)
            ...
            play_req = await _read_until(new_ws, protocol.PLAY_REQUEST)
            assert play_req["legal_cards"]
            card = play_req["legal_cards"][0]
            await _send(new_ws, protocol.PLAY_CARD, {"card": card})
            played = await _read_until(observer, protocol.CARD_PLAYED)
            assert played["seat"] == "E"

            # Finish trick 1 with N's final play (W, S, E already played above).
            req = await _read_until(conns["N"], protocol.PLAY_REQUEST)
            card = req["legal_cards"][0]
            await _send(conns["N"], protocol.PLAY_CARD, {"card": card})
            played = await _read_until(observer, protocol.CARD_PLAYED)
...
def test_out_of_turn_bid_rejected_with_error_and_game_unaffected():
    async def scenario() -> None:
        ...
        try:
            ...
            await _send(conns["S"], protocol.BID, {"action": "pass"})
            error_payload = await _read_any_until(conns["S"], protocol.ERROR)
            assert error_payload["code"] == protocol.NOT_YOUR_TURN

            # Game state must be unaffected: W can still bid normally afterward.
            action = bid_req["legal_actions"][0]
            await _send(
                conns["W"], protocol.BID, {"action": "bid", "trump": action["trump"], "points": action["points"]}
            )
            update = await _read_until(observer, protocol.BID_UPDATE)
            assert update["seat"] == "W" and update["action"] == "bid"
        finally:
...
def test_out_of_turn_card_play_rejected_with_error_and_game_unaffected():
    async def scenario() -> None:
        ...
        try:
            ...
            play_req = await _read_until(conns["W"], protocol.PLAY_REQUEST)

            # S attempts to play before it is their turn (W is first leader).
            await _send(conns["S"], protocol.PLAY_CARD, {"card": "7♠"})
            error_payload = await _read_any_until(conns["S"], protocol.ERROR)
            assert error_payload["code"] == protocol.NOT_YOUR_TURN

            # Game state must be unaffected: W can still lead normally afterward.
            card = play_req["legal_cards"][0]
            await _send(conns["W"], protocol.PLAY_CARD, {"card": card})
            played = await _read_until(observer, protocol.CARD_PLAYED)
...
def test_illegal_card_play_rejected_with_error_and_state_unchanged(monkeypatch):
    ...
    async def scenario() -> None:
        ...
        try:
            ...
            await _read_until(conns["W"], protocol.DEAL)
            await _bid_min_and_finalize_contract(conns, observer)  # W bids 80 ♠

            w_play_req = await _read_until(conns["W"], protocol.PLAY_REQUEST)
            w_card = w_play_req["legal_cards"][0]
            assert w_card == "7♠"
            await _send(conns["W"], protocol.PLAY_CARD, {"card": w_card})
            await _read_until(observer, protocol.CARD_PLAYED)

            # S must follow/overtrump in spades; R♥ (a heart S holds) is illegal.
            s_play_req = await _read_until(conns["S"], protocol.PLAY_REQUEST)
            assert "R♥" not in s_play_req["legal_cards"]
            await _send(conns["S"], protocol.PLAY_CARD, {"card": "R♥"})
            error_payload = await _read_any_until(conns["S"], protocol.ERROR)
            ...
            await _send(conns["S"], protocol.PLAY_CARD, {"card": legal_card})
            played = await _read_until(observer, protocol.CARD_PLAYED)
...
def test_empty_card_string_rejected_with_error_not_a_crash():
    ...
    async def scenario() -> None:
        ...
        try:
            ...
            play_req = await _read_until(conns["W"], protocol.PLAY_REQUEST)
            await _send(conns["W"], protocol.PLAY_CARD, {"card": ""})
            error_payload = await _read_any_until(conns["W"], protocol.ERROR)
            assert error_payload["code"] == protocol.ILLEGAL_CARD

            # Connection must still be usable afterward (no crash/hang).
            legal_card = play_req["legal_cards"][0]
            await _send(conns["W"], protocol.PLAY_CARD, {"card": legal_card})
            played = await _read_until(observer, protocol.CARD_PLAYED)
            assert played["seat"] == "W"
        finally:
            for websocket in conns.values():
...
def test_malformed_json_line_rejected_without_crashing_connection():
    async def scenario() -> None:
        ...
        try:
            ...
            await _read_until(websocket, protocol.DEAL)
            await _read_until(other, protocol.DEAL)

            await websocket.send(b"not valid json at all")
            error_payload = await _read_any_until(websocket, protocol.ERROR)
            assert error_payload["code"] == protocol.MALFORMED_MESSAGE

            # Connection must still be usable afterward.
            await _send(websocket, protocol.CHAT, {"text": "still alive"})
            chat_seen = await _read_until(other, protocol.CHAT)
...
def test_oversized_message_is_rejected_gracefully_not_a_server_crash():
    """Regression test: a client message exceeding the server's configured
    `max_size` used to raise an unhandled ValueError from the old raw-TCP
    StreamReader.readline(), crashing the connection task instead of being
    rejected cleanly. The WebSocket transport now enforces this limit itself
    (RFC 6455 close code 1009, "message too big") and simply closes the
    connection -- the server must stay healthy for the next connection.
    See discoveries.md DISC-009."""

    async def scenario() -> None:
        srv, port = await _start_server()
        try:
            websocket = await _connect(port)
            huge_name = "A" * 70_000  # exceeds the server's configured max_size (64 KiB)
            await _send(websocket, protocol.JOIN, {"table_key": "oversz1", "player_name": huge_name})

            with pytest.raises(ConnectionClosed):
                await asyncio.wait_for(websocket.recv(), timeout=2.0)

            # The server itself must still be healthy afterward.
            websocket2 = await _connect(port)
            await _send(websocket2, protocol.JOIN, {"table_key": "oversz2", "player_name": "Zoe"})
            mtype2, payload2 = await _recv(websocket2)
...
def test_trick_completion_pauses_before_next_play_request():
    ...
    async def scenario() -> None:
        async def _handler(websocket: ServerConnection) -> None:
            await server.handle_connection(websocket, 1000, trick_pause_seconds=0.3, round_pause_seconds=0)

        srv = await serve(_handler, HOST, 0)
        port = srv.sockets[0].getsockname()[1]
        conns: dict = {}
        try:
            ...
            await _read_until(conns[winner], protocol.PLAY_REQUEST)
...
def test_round_completion_pauses_before_next_deal():
    ...
    async def scenario() -> None:
        async def _handler(websocket: ServerConnection) -> None:
            await server.handle_connection(websocket, 1000, trick_pause_seconds=0, round_pause_seconds=0.3)
        ...
        try:
            ...
            start = asyncio.get_event_loop().time()
            await _read_until(observer, protocol.ROUND_SCORE)
            await _read_until(observer, protocol.DEAL)
            elapsed = asyncio.get_event_loop().time() - start
...
def test_rematch_after_game_over_restarts_with_fresh_scores():
    ...
    async def scenario() -> None:
        ...
        try:
            ...
            await _read_until(observer, protocol.ROUND_SCORE)
            game_over = await _read_until(observer, protocol.GAME_OVER)
            assert game_over["winning_team"] in ("NS", "EW")

            # N asks for a rematch.
            await _send(conns["N"], protocol.REMATCH, {})

            new_game = await _read_until(observer, protocol.NEW_GAME)
...
def test_fifth_player_join_rejected_game_in_progress():
    async def scenario() -> None:
        ...
        try:
            ...
            ws5 = await _connect(port)
            await _send(ws5, protocol.JOIN, {"table_key": "full01", "player_name": "Eve"})
            mtype, payload = await _recv(ws5)
...
def test_duplicate_name_join_rejected_name_taken_case_insensitive():
    async def scenario() -> None:
        ...
        try:
            ...
            await _send(ws1, protocol.JOIN, {"table_key": "dup01", "player_name": "Alice"})
            mtype1, _ = await _recv(ws1)
            assert mtype1 == protocol.JOINED
            conns["N"] = ws1

            ws2 = await _connect(port)
            await _send(ws2, protocol.JOIN, {"table_key": "dup01", "player_name": "alice"})
            mtype2, payload2 = await _recv(ws2)
...
def test_disconnect_and_reconnect_during_bidding_phase():
    async def scenario() -> None:
        ...
        try:
            ...
            await _read_until(conns[next_seat], protocol.BID_REQUEST)
            dropped_websocket = conns[next_seat]
            await dropped_websocket.close()
            ...
            new_ws = await _connect(port)
            await _send(new_ws, protocol.JOIN, {"table_key": "reconbid01", "player_name": NAMES_BY_SEAT[next_seat]})
            ...
            conns[next_seat] = new_ws

            # It's still next_seat's turn: the server follows resync with a bid_request.
            bid_req2 = await _read_until(new_ws, protocol.BID_REQUEST)
            assert bid_req2["legal_actions"]

            # Finish the auction (S, E, N all pass) to confirm resumed bidding works.
            await _send(new_ws, protocol.BID, {"action": "pass"})
            update2 = await _read_until(observer, protocol.BID_UPDATE)
            seat_ptr = update2["next_to_act"]  # "E"

            await _read_until(conns[seat_ptr], protocol.BID_REQUEST)
            await _send(conns[seat_ptr], protocol.BID, {"action": "pass"})
            update3 = await _read_until(observer, protocol.BID_UPDATE)
            seat_ptr = update3["next_to_act"]  # "N"

            await _read_until(conns[seat_ptr], protocol.BID_REQUEST)
            await _send(conns[seat_ptr], protocol.BID, {"action": "pass"})
            bidding_result = await _read_until(observer, protocol.BIDDING_RESULT)
