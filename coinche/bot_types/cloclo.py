...
class ClocloBot(BotType):
    ...
    def choose_bid(self, game: Game, seat: Seat) -> dict:
        ...
        if not affordable_bids:
            return {"action": "pass"}
        return max(affordable_bids, key=lambda action: self._ceiling_rank(action["points"]))
    ...
    def choose_card(self, game: Game, seat: Seat) -> Card:
        ...
        winning_cards = [
            ...
            if not trick or TEAM_OF[rules.trick_winner([*trick, (seat, card)], trump)] == TEAM_OF[seat]
        ]
        if winning_cards:
            return min(winning_cards, key=lambda card: self._winning_key(card, trump))
        return min(legal_cards, key=lambda card: self._discard_key(card, trump))
    ...
    @staticmethod
    def _winning_key(card: Card, trump: str) -> tuple[int, bool, int, str]:
        order = rules.TRUMP_ORDER if card.suit == trump else rules.NONTRUMP_ORDER
        return (rules.card_points(card, trump), card.suit == trump, order.index(card.rank), card.suit)
